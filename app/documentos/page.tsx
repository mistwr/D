      ...
      <div className="flex">
        ...
        <main className="flex-1 md:ml-64 pt-16" style={{ minHeight: '100vh' }}>
          <div className="p-4 md:p-8">
            <h1 className="text-2xl font-bold" style={{ color: '#111827' }}>Documentos e Contratos</h1>
            <p className="mt-1 mb-6 text-sm" style={{ color: '#6b7280' }}>Carregue e consulte documentos por venda</p>
            ...
            {vendas.length === 0 ? (
              <div className="rounded-xl p-12 text-center shadow-sm" style={{ background: '#fff', border: '1px solid #e5e7eb' }}>
                ...
                <p className="text-lg font-medium" style={{ color: '#374151' }}>Sem vendas registadas</p>
                <p className="mt-1 text-sm" style={{ color: '#6b7280' }}>Registe uma venda primeiro</p>
              </div>
            ) : (
                      ...
                      <button onClick={() => setExpandedVenda(isExpanded ? null : v.id)}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs" style={{ color: '#6b7280' }}>{v.operator} - {v.service_type === 'energia' ? 'Energia' : 'Telecom'} - {vendaDocs.length} doc(s)</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          {isExpanded ? <ChevronUp size={18} style={{ color: '#6b7280' }} /> : <ChevronDown size={18} style={{ color: '#6b7280' }} />}
                        </div>
                      </button>
                            ...
                            <div className="flex flex-col gap-2 mb-4">
                              {vendaDocs.map(d => (
                                <div key={d.id} className="flex items-center justify-between rounded-lg px-4 py-3" style={{ background: '#f9fafb', border: '1px solid #f3f4f6' }}>
                                  <div className="flex items-center gap-3">
                                    <span className="flex h-8 w-10 items-center justify-center rounded text-xs font-bold" style={{ background: '#e0e7ff', color: '#4338ca' }}>
                                      {typeExt[d.file_type] || d.file_name.split('.').pop()?.toUpperCase() || 'DOC'}
                                    </span>
                                    <div>
                                      <p className="text-sm font-medium" style={{ color: '#111827' }}>{d.file_name}</p>
